...
Для Windows используйте `one_tap_windows.bat` из готового release bundle с Windows-бинарниками. Скрипт сам создаёт `windows\winws2.args`, проверяет стратегии на YouTube/Telegram, сохраняет рабочую стратегию в `windows\strategy.windows.args` и ставит сервис `winws2`.
