...
Для автоматической установки и запуска на Linux/OpenWrt используйте [one_tap.sh](one_tap.md):

```
sudo ./one_tap.sh
```

Для Windows используйте `one_tap_windows.bat` из готового release bundle с Windows-бинарниками. Скрипт сам создаёт `windows\winws2.args`, проверяет стратегии на обычном HTTPS-сайте и реальных страницах/ассетах YouTube/Telegram/Discord, подбирает hostlist-scoped TLS/media профили, дропает целевой QUIC для браузерного TCP-fallback, добавляет Telegram DC ipset для Desktop-клиента, сохраняет рабочую стратегию в `windows\strategy.windows.args`, оставляет лучшую безопасную деградированную стратегию для ручной проверки либо восстанавливает прошлую стратегию после неудачного перебора и ставит сервис `winws2`.

macOS-версия вынесена в `macos/one_tap_macos.sh --self-test`: она проверяет и собирает поддерживаемые user-space утилиты. Прозрачный DPI-перехват на macOS в текущей архитектуре невозможен, потому что в современных macOS нет подходящего kernel divert механизма для `nfqws2`/`dvtws2`.

Подробности: [docs/one_tap.md](one_tap.md).
