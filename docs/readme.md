...
Для Windows используйте `windows\one_tap_windows.cmd` из готового release bundle с Windows-бинарниками. Скрипт сам создаёт `windows\winws2.args`, сохраняет стратегию в `windows\strategy.windows.args` и ставит сервис `winws2`.
